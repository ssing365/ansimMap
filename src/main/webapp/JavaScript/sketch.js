...
measureLength.addEventListener("click", () => {
    ...
    if (areaFlag) {
        measureExtent.classList.remove('clicked');
        areaFlag = false;
...
// 넓이 측정
const measureExtent = document.getElementById("measureExtent");
let areaFlag = false;
measureExtent.addEventListener("click", () => {

    // measureLength 버튼의 상태를 초기화
    if (lengthFlag) {
        measureLength.classList.remove('clicked');
        lengthFlag = false;
        map.removeInteraction(draw);
        source2.clear();
        const elements = document.getElementsByClassName("ol-tooltip ol-tooltip-static");
